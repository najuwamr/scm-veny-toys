...
<div class="ml-60 flex-1 p-8">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-slate-900">Tambah Penawaran Bahan Baku</h1>
            <p class="text-sm text-slate-500">Tambahkan atau perbarui harga bahan baku yang Anda tawarkan.</p>
        </div>
        <a href="{{ route('supplier.offers.index') }}" class="inline-flex items-center rounded-full bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800">
            Kembali ke Daftar Penawaran
        </a>
    </div>

    <div class="rounded-3xl bg-white p-6 shadow-sm">
        <form action="{{ route('supplier.offers.store') }}" method="post" class="grid gap-6">
            @csrf
            <div>
                ...
                <select name="bahan_baku_id" class="w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-pink-500 focus:outline-none">
                    ...
                    @foreach($bahanBakus as $bahan)
                        <option value="{{ $bahan->id }}" {{ old('bahan_baku_id') == $bahan->id ? 'selected' : '' }}>{{ $bahan->nama }} ({{ $bahan->satuan }})</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <button type="submit" class="inline-flex w-full items-center justify-center rounded-3xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white hover:bg-slate-800">Simpan Penawaran</button>
        </form>
    </div>
</div>
@endsection
