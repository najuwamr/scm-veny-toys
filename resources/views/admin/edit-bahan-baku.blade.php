...
<main class="ml-60 flex-1 p-8">
    <div class="mb-8">
        <div>
            ...
            <p class="text-sm text-gray-400">Perbarui informasi bahan baku yang ada.</p>
        </div>
    </div>

    <div class="rounded-2xl bg-white shadow-sm border border-pink-100 p-6">
        <form action="{{ route('admin.inventory.bahan.update', $bahan) }}" method="POST" class="grid gap-4 md:grid-cols-2">
            @csrf
            @method('PUT')

            <div>
                <label class="mb-1 block text-sm font-semibold text-gray-700">Kode Bahan</label>
                <input type="text" name="kode_bahan" value="{{ old('kode_bahan', $bahan->kode_bahan) }}" required class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-pink-400 focus:outline-none" placeholder="BB-001">
            ...
            <div>
                ...
                <input type="text" name="nama_bahan" value="{{ old('nama_bahan', $bahan->nama_bahan) }}" required class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-pink-400 focus:outline-none" placeholder="Kain Rasfur">
            ...
            <div>
                ...
                <select name="kategori" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-pink-400 focus:outline-none">
                    ...
                    <option value="kain" {{ old('kategori', $bahan->kategori) == 'kain' ? 'selected' : '' }}>Kain</option>
                    <option value="isi boneka" {{ old('kategori', $bahan->kategori) == 'isi boneka' ? 'selected' : '' }}>Isi Boneka</option>
                    <option value="aksesoris" {{ old('kategori', $bahan->kategori) == 'aksesoris' ? 'selected' : '' }}>Aksesoris</option>
                </select>
            ...
            <div>
                ...
                <select name="satuan" required class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-pink-400 focus:outline-none">
                    ...
                    <option value="meter" {{ old('satuan', $bahan->satuan) == 'meter' ? 'selected' : '' }}>Meter</option>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="stok_saat_ini" value="{{ old('stok_saat_ini', $bahan->stok_saat_ini) }}" required class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-pink-400 focus:outline-none">
            ...
            <div>
                ...
                <input type="number" step="0.01" name="stok_minimum" value="{{ old('stok_minimum', $bahan->stok_minimum) }}" required class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-pink-400 focus:outline-none">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="keterangan" rows="3" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-pink-400 focus:outline-none">{{ old('keterangan', $bahan->keterangan) }}</textarea>
